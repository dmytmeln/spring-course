...
| `/api/reader/reserve/{book_id}` | POST | Reserve a book that is currently unavailable | None | `book_id`: int | `200 OK` - `{ "reservation_id": "int", "book_id": "int", "reader_ticket_id": "int", "start_date": "date", "end_date": "date" }` |
| `/api/reader/loans/{book_id}` | POST | Borrow books from the library | None | `book_id`: int | `200 OK` - `[ { "loan_id": "int", "book_copy_id": "int", "reader_ticket_id": "int", "start_date": "date", "end_date": "date", ]` |
